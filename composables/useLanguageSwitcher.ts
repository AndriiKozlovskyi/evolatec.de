 ...
 * Keys and values are WITHOUT trailing slashes (norm() is applied at lookup).
...
export function useLanguageSwitcher() {
  ...
  const deHref = computed(() =>
    isEnglish.value ? (enToDe[norm(route.path)] ?? '/') : route.path
  )

  const enHref = computed(() =>
    isEnglish.value ? route.path : (deToEn[norm(route.path)] ?? '/en/')
  )
